    ...
    <View
      ...
      <View style={{textAlign: 'left'}}>
        ...
        <View style={designs.rlDisplay}>
          ...
          <CustomTextInput onPress={onPressB} pressed={pressed} placeHolderText="Email"/>
          <CustomPicker onPress={onPress} pressed={secondPressed} value={selectedOption} close={()=>{setPickerModalVisible(!pickerModalVisible);
              setSecondPressed(!secondPressed)}} modalVisible ={pickerModalVisible}>
          <View>
            { loanPurpose.map((value, index) => {
              return <TouchableOpacity key={index} onPress={() => {setSelectedOption(value.value); setPickerModalVisible(false); setSecondPressed(!secondPressed)}}>
        ...
        <TouchableOpacity style={[designs.buttonStyleB, {backgroundColor: '#00DC99', width: '100%'}]} onPress={() => navigation.navigate('AccountPage')}><Text style={{color: COLORS.white}}>ACCESS LOAN</Text></TouchableOpacity>
        </ScrollView>
        </View>
    </View>
  );
