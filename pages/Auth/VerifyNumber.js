...
import {images,COLORS} from '../../util/index';
  ...
  const DisplayCountDownTimer=()=>{
    ...
    <>
      <View style={designs.counter}>
        ...
        />
        <Text style={{color:'#fff',marginLeft:5}}>sec</Text>
      </View>
    </>
  )};


  const DisplayResentOTP=()=>  {return (
    <>
     <TouchableOpacity onPress={resendOTP} style={{ alignText:'center',flexDirection:'row',justifyContent:'center',marginTop:52}}>
       <Text style={[{alignText:'center',
        color: '#2A286A',
        fontFamily: 'CircularStd',}]}>Didn't receive OTP or it expired</Text> 
       <Text style={[{color:COLORS.light,fontFamily: 'CircularStd'}]}> Resend</Text> 
     </TouchableOpacity>
    </>
  )};
